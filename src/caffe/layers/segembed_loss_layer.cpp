...
class SegEmbedAffinityGraphCompare {
 private:
  const Dtype * mEdgeWeightArray;
 public:
  explicit SegEmbedAffinityGraphCompare(const Dtype * EdgeWeightArray) {
...
void SegEmbedLossLayer<Dtype>::SegEmbed(const Dtype* conn_data,
  ...
  std::sort(pqueue.begin(), pqueue.end(),
       SegEmbedAffinityGraphCompare<Dtype>(conn_data));
...
template<typename Dtype>
void SegEmbedLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
template<typename Dtype>
void SegEmbedLossLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
                                    const vector<Blob<Dtype>*>& top) {
...
template<typename Dtype>
void SegEmbedLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
                                        const vector<Blob<Dtype>*>& top) {
...
// square loss (euclidean) is used by SEGEMBED
...
#pragma omp parallel for reduction(+:loss)
  for (int_tp batch = 0; batch < bottom[0]->shape()[0]; ++batch) {
    ...
    SegEmbed(&affinity_data_neg[batch_offset * batch], conn_num_dims_,
    ...
    SegEmbed(&affinity_data_pos[batch_offset * batch], conn_num_dims_,
          &conn_dims_[0], &nhood_data_[0], &nhood_dims_[0],
...
template<typename Dtype>
void SegEmbedLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
